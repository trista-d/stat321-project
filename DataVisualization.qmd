...
Note that this is after capping upper and lower outlier salaries using the IQR method and then plotting only the lowest 80 salary values, as this
...
```{r}
ggplot(capcut_nfl, aes(x = Yds, y = capSalary)) +
  ...
  geom_vline(xintercept = mean(capcut_nfl$Yds), color = "grey") +
  theme_minimal() +
...
```{r}
ggplot(final_clean_nfl, aes(x = catchRate, y = logSalary)) +
  geom_point(size = 3, color = '#56B4E9') +
  labs(title = "log(Salary) vs. Catch Rate",
       x = "Catch Rate",
       y = "log(2024 Salary ($))") +
  ylim(13, 17.5) +
  # add a vertical line at the median Yds
  geom_vline(xintercept = median(final_clean_nfl$catchRate), color = "grey") +
  theme_minimal()
  # highlight some of the highest salaries and some of the highest yards travelled
```
