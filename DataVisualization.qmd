...
# Plots based on Trista's modelling results
...
```{r}
ggplot(final_clean_nfl, aes(x = Rec.YAC.R, y = catchRate)) +
  geom_point(size = 3, color = '#D55E00') +   # size is for size of the points
  labs(title = "Catch Rate vs. Rec.YAC.R",
       x = "Rec.YAC.R",
       y = "Catch Rate") +
  ylim(0.4, 1) +
  xlim(60, 800) +
  # add a vertical line at the median TD
  geom_vline(xintercept = mean(final_clean_nfl$Rec.YAC.R), color = "grey") +
  theme_minimal() +
  # label the players with highest number of touchdowns
  geom_text(label = ifelse(final_clean_nfl$Rec.YAC.R > 550, final_clean_nfl$Player, ''), hjust = 0.7, vjust = -1) +
  annotate("text", label = "Mean Rec.YAC.R", x = mean(final_clean_nfl$Rec.YAC.R), y = 1, color = "grey")
```
...
ggplot(final_clean_nfl, aes(x = Yds, y = Rec)) +
  ...
  annotate("text", label = "Mean Yds", x = mean(final_clean_nfl$Yds), y = 120, color = "grey")
```
...
```{r}
ggplot(final_clean_nfl, aes(x = Rec.1st, y = TD)) +
  geom_point(size = 3, color = '#D55E00') +   # size is for size of the points
  labs(title = "TD vs. Rec.1st",
       x = "Rec.1st",
       y = "TD") +
  ylim(0, 18) +
  xlim(10, 80) +
  # add a vertical line at the median TD
  geom_vline(xintercept = mean(final_clean_nfl$Rec.1st), color = "grey") +
  theme_minimal() +
  # label the players with highest number of touchdowns
  geom_text(label = ifelse(final_clean_nfl$Rec.1st > 60, final_clean_nfl$Player, ''), hjust = 0.7, vjust = -1) +
  annotate("text", label = "Mean Rec.1st", x = mean(final_clean_nfl$Rec.1st), y = 17, color = "grey")
```
