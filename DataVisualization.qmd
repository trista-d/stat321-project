...
ggplot(bottom10_TD_nfl, aes(x = factor(TD), y = Salary)) +
  ...
  labs(title = "Average 2024 Salary of Players with 10 Fewest Touchdowns", x = "Touchdowns", y = "2024 Salary ($)") + 
  theme_minimal()

# and 10 most touchdowns. 
ggplot(top10_TD_nfl, aes(x = factor(TD), y = Salary)) +
  geom_bar(stat = "summary", fun = "mean", fill = '#D55E00') +
  labs(title = "Average 2024 Salary of Players with 10 Highest Touchdowns", x = "Touchdowns", y = "2024 Salary ($)") + 
  theme_minimal()
```
...
# plotting bar chart of salaries for the players who made the 10 least touchdowns. 
ggplot(bottom10_Yds_nfl, aes(x = factor(Yds), y = Salary)) +
  geom_bar(stat = "summary", fun = "mean", fill = '#56B4E9') +
  labs(title = "Average 2024 Salary of Players with 10 Fewest Yards", x = "Total Yards", y = "2024 Salary ($)") + 
  theme_minimal()
...
#### Plot 3 alternatives: Bar charts of Salary for all touchdowns or yards. 

Instead tried just plotting average salary for each number of touchdowns or yards so we can see everything at once. I think this would be a bit more useful.
...
```{r}
ggplot(sorted_Yds_nfl, aes(x = Yds, y = Salary)) +
  geom_bar(stat = "summary", fun = "mean", fill='#D55E00') +
  labs(title = "Average 2024 Salary by Yards", x = "Total Yards", y = "2024 Salary ($)") + 
  theme_minimal()
```

From this we can tell that both high and low salaries are present at each number of total yards. Need to fix the x axis though.
